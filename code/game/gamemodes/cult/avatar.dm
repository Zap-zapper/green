var/avatarcreated=0		//Áûë ëè óæå ñîçäàí Àâàòàð?
var/mob/living/simple_animal/avatar/demon		//Ãëàâíàÿ ïåðåìåííàÿ, íà êîòîðîé îñíîâàí àâàòàð, åãî àáèëêè, è âñ¸-âñ¸-âñ¸.

/proc/messagevictimcheck(mob/living/P as mob)		//Ïðîâåðêà íà öåëü ñ ñîîáùåíèåì
...
/proc/victimcheck(mob/living/M as mob)		//Ïðîâåðêà íà öåëü áåç ñîîáùåíèé
...
/mob/living/simple_animal/avatar
	...
	see_in_dark = 8
	environment_smash = 3
	mob_size = MOB_SIZE_LARGE		//×òîáû â øêàô÷èêå íå çàâàðèëè
	a_intent = "harm"
	...
	health = 400
	speed=5
	status_flags = 0		//Íåëüçÿ òîëêíóòü
	...
	minbodytemp = 0
	maxbodytemp = INFINITY		//Ïîôèã íà àòìîñ. Âîçìîæíî ñòîèò óáðàòü, èáî ïî÷åìó áû íå äàòü âîçìîæíîñòü åãî çàæàðèòü/êîñìèðîâàòü?
	atmos_requirements = list("min_oxy" = 0, "max_oxy" = 0, "min_tox" = 0, "max_tox" = 0, "min_co2" = 0, "max_co2" = 0, "min_n2" = 0, "max_n2" = 0)
	faction = list("faithless","cult")		//×òîáû åãî íîãàìè íå çàïèíàëè åãî æå ñîáñòâåííûå äåìîíû
	var/stepsound=0		//Ñêîëüêî øàãîâ äî çâóêà?
	var/enraged=0		//Â ÿðîñòè?
	var/victim=null		//Æåðòâà ïîä âîçäåéñòâèåì Doom. Íóæíà äëÿ òîãî ÷òîáû ïðîêëÿòüå ìîæíî áûëî ñêèíóòü óäàðèâ áèáëèåé, à òàê æå îïðåäåëèòü åñòü ëè îíà âîîáùå â ìèðå
	var/hunger=200		//Need...moar...souls
	var/summoned_during_cult=0		//Åãî ïðèçâàëè âî âðåìÿ êóëüòà?
	var/attack_cooldown=2		//Êóëäàóí àòàêè
...
//////////////////////// ÃËÀÂÍÛÅ ÏÐÎÊÈ ////////////////////////
	...
	New()
		..()
		if (avatarcreated || demon)		//Åñòü òîëüêî îäèí Àâàòàð.
			spawn(1)		message_admins("There can be only one Avatar in the world")
			qdel(src)

		demon=src
		avatarcreated = 1		//Ïåðåìåííóþ íåëüçÿ óñòàíîâèòü íà 0 áåç ïåäàëüâìåøàòåëüñòâà. Åñëè àâàòàð ñîñíóë, òî íîâîãî íå ñäåëàòü.
		if(ticker.mode.name == "cult")		summoned_during_cult=1

		var/matrix/M = matrix()		//Óâåëè÷èòü ñïðàéò Àâàòàðà. Ýòî äåëàåò åãî ìûëüíûì, íî òóò óæå âûáîð - ëèáî êàðëèêîâûé íî íîðìàëüíûé, ëèáî íîðìàëüíûé, íî ìûëüíûé.
		M.Scale(1.25)
		M.Translate(0,5)
		src.transform = M
		...
		if(!iscultist(src) && summoned_during_cult && mind)
			...
			src.mind.current.verbs -= /mob/living/proc/cult_innate_comm		//Åìó ýòî íå íóæíî

		spawn(1)
			if(!src.giveSpells())		//Íàãëî óêðàäåíî ó ðåâåíàíòà
	...
	Stat()		//Còàò-ïàíåëüêà ñ âðåìåíåì è ãîëîäîì
		..()
	...
	Life()
		...
		for(var/mob/living/M in view(src.loc,3))		//Åãî àóðà
			if(M)
				if(victimcheck(M))
		...
		if(health <=200 && enraged==0)		//ßðîñòü
...
/mob/living/simple_animal/avatar/proc/giveSpells()		//Ïðîê, êîòîðûé âûäà¸ò åìó àáèëêè. Íàãëî óêðàäåíî ó Ðåâåíàíòà.
	if(src.mind)
...
/obj/effect/proc_holder/spell/targeted/soul_absorb/cast(list/targets)
	for(var/mob/living/carbon/Q in targets)
		spawn(0)
			if(!Q)		return
			if(!Q.ckey && !Q.get_ghost())		//Âîðâàòüñÿ â ãåíåòèêó, ïåðåæðàòü ìàêàê è ïîáåäèòü? Íåm.
			...
			demon.adjustBruteLoss(-50)		//Îòíÿòü âðåìÿ îò îòñ÷¸òà, ïîõèëèòü àâàòàðà è ñäåëàòü åãî ìåíåå ãîëîäíûì.
			demon.hunger=200
...
/obj/effect/proc_holder/spell/aoe_turf/summon_daemons/cast(list/targets)		//Conjure - äåðüìî.
	var/list/faithlesses= list()
	var/time=300		//Âðåìÿ ñêîëüêî îíè ñóùåñòâóþò
	if(demon.enraged)		time=150

	for(var/turf/target_tile in targets)
		if(!target_tile)		return
		for(var/mob/living/M in target_tile.contents)		//Ïèõíóòü íàçàä òåõ, êòî ðÿäîì
			if(M!=demon)
				M << "\red You was pushed away by appearing figures!"
				step_away(M,demon)
				M.Weaken(2)
		if(!istype(target_tile,/turf/space) && !(is_blocked_turf(target_tile)))		//Åñëè â ÿðîñòè, òî âîñåìü äåìîíîâ ïîÿâÿòñÿ âîêðóã. Åñëè íåò, òî ÷åòûðå, ñëåâà,ñïðàâà, ñïåðåäè è ñçàäè.
...
/obj/effect/proc_holder/spell/targeted/suffer/cast(list/targets)
	for(var/mob/living/T in targets)
		...
		demon.say("COGNOSCE DOLOR")		//Åñëè åáàí¸ò íå òîãî - âñ¸ ðàâíî íà êóëäàóí
...
/obj/effect/proc_holder/spell/targeted/inflict_handler/harpoon
	amt_weakened = 4
	amt_dam_brute = 25		//Ïîïàäàíèå ãàðïóíîì âàëèò ñ íîã è íàíîñèò 25 áðóòà
	...
	spawn(0)
		for(var/mob/living/jertva in targets)
			...
			while(get_dist(demon,jertva)>1 && i<=10)
				++i
				jertva.loc=get_step_towards(jertva,demon)		//×òîáû ÷åðåç ñòîëû ïðîëåòàëà. Åñëè ìåæäó íèìè îêàæåòñÿ ñòåíà, òî ýòî òèï ò¸ìíàÿ ñèëà, ìèñòèêà, êîðî÷ íå áàã à ôè÷à
				sleep(2)
	...
	spawn(0)
		...
		for(var/turf/T in targets)
			...
			for(var/mob/living/K in T.contents)
				...
				if(istype(K,/mob/living/silicon))		//Áîðãîòó òóäà æå
			...
			for(var/obj/structure/window/W in T.contents)		if(prob(33))		W.spawnfragments()		//Ðèñê ëàãàëèùà, ïîýòîìó òóò øàíñ âñåãäà 1/3


//// Doom


/mob/living/proc/Doom(var/countdown = 60)		//×åðåç îòäåëüíûé ïðîê
	...
	spawn(1)
		...
		while(countdown>0)
			...
			--countdown
			switch(countdown)		//Áèáëåéñêèé ñòèøîê. Âîçìîæíî íå ñàìîå ýïè÷íîå èëè ïîäõîäÿùåå, íî ïî ìíå íîðì.
				...
				if(-INFINITY to 0)
					for(var/mob/living/K in oview(5, src))		//Ïðûæîê íà ñëó÷àéíóþ öåëü ðÿäîì
...
/obj/effect/proc_holder/spell/aoe_turf/inferno/cast(list/targets)
	...
	for(var/turf/simulated/T in range(demon,7))
		...
		if(get_dist(T,stone)==7)
			var/obj/effect/forcefield/flames = new /obj/effect/forcefield(T)		//Íèêòî íè÷åãî íå çàïîäîçðèò...
			flames.name= "hotspot"
			...
			flames.luminosity = 5
			walls+=flames
		else		T.color="#383838"		//Âñ¸ ÷òî âíóòðè ñòàíåò ÷¸ðíûì
...
/mob/living/simple_animal/avatar/ex_act(severity)		//Íó åáàíóòü îäíîé ëèìèòêîé, íó âåñü ôàí æå íàõóé, íó.
